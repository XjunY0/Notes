...
2.  **导出模型到 `onnx`**：
    ...
    `import torch.onnx torch.onnx.export(model, dummy_input, "model.onnx")`
...
3.  **加载和运行 `onnx` 模型**：
    ...
    `import onnxruntime as ort ort_session = ort.InferenceSession("model.onnx") outputs = ort_session.run(None, {"input": input_data})`
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTgxMzkwODg0NywtMTgyMjI5NDM3N119
-->